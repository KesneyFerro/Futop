...
const OpportunityPage: NextPage = ({ post }: any) => {
  ...
  const expirationDate = (date: string, time: string) => {
  ...
  const regex = /-/g;
  const tags = post.posts.tags.map((string: string) => string.trim());
  return (
    <HomeCointainer>
      <Head>
        <link
        ...
        <meta
          ...
          content={`${
            post.posts.texts != undefined ? post.posts.texts[0] : t("nopost")
          }`}
        />
        ...
        />
        <meta property="og:article:author" content={`${post.posts.title}`} />
